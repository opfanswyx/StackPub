...
**析构函数**定义类对象销毁时做什么。
#### 拷贝构造函数
如果一个构造函数的第一个参数是自身类类型的引用，且任何参数都有默认值,则此构造函数是**拷贝构造函数**。拷贝构造函数在几种情况下都会被隐式调用。因此，拷贝构造函数不应该是explicit的。
```cpp
class Foo{
  Foo();            //默认构造函数
...
5. 某些类类型还会对它们所分配的对象使用拷贝初始化。

在拷贝构造函数初始化过程中，编译器可以跳过拷贝/移动构造函数，直接创建对象。
#### 拷贝赋值运算符
...
#### =default
我们可以通过将拷贝控制成员定义为=default来显示的要求编译器来生成合成的版本。
```cpp
class sales_data{
public:
  sales_data() = default;
  sales_data(const sales_data&) = default;
  sales_data& operator=(const salesdata &);
  ~sales_data() = default;
  //....
};
sales_data& sales_data::operator=(const sales_data&) = default;
```
当我们在类内用=default修饰成员的声明时，合成的函数将隐式地声明为内联的。如果不希望合成的成员是内联函数，应该只对成员的类外定义使用=default。
#### 阻止拷贝
可以通过将拷贝构造函数和拷贝赋值运算符定义为删除的函数(deleted function)来阻止拷贝。虽然声明了它，但不能使用任何方式使用它。
```cpp
struct nocopy{
  nocopy() = default;
