 ...
 * Parameters: the socket connected to the client(连接到客户端的套接字)
***********************************************************************/
void accept_request(void *arg)
{ 
    int client = (intptr_t)arg; /* intptr_t在不同的平台是不一样的，始终与地址位数相同 */
    char buf[1024];
    size_t numchars;
    char method[255];
    char url[255];
    ...
    j=i;
    method[i] = '\0';
    printf("LINE[%d]\tfunction[%s]\tmethod=[%s]\n",__LINE__,__FUNCTION__,method);
    /************************************
    ...
    { 
        unimplemented(client);  /* 如果接收到的数据中既有GET方法与POST方法，则向客户端返回该web方法尚未实现 */
        return;
    }
    ...
    **********************************/
    ...
    if (strcasecmp(method, "GET") == 0)
    {
        ...
        while ((*query_string != '?') && (*query_string != '\0'))   /* 如果是GET请求，url可能会带有?,有查询参数 */
            query_string++;
        if (*query_string == '?')
        {
            cgi = 1;          
    ...
    sprintf(path, "htdocs%s", url);     /* url中的路径格式化到path */
    ...
    else
    {
        ...
        ***************************************/
        ...
        if ((st.st_mode & S_IXUSR) ||           /* S_IXUSR:文件所有者具可执行权限 */
                (st.st_mode & S_IXGRP) ||       /* S_IXGRP:用户组具可执行权限 */
                (st.st_mode & S_IXOTH)    )     /* S_IXOTH:其他用户具可读取权限 */     
            cgi = 1;
        if (!cgi)    
            serve_file(client, path);   /* 读取静态文件返回给请求的http客户端 */
        else
            execute_cgi(client, path, method, query_string);     /* 执行cgi动态解析 */
    }
    close(client);          /* 执行完毕后，关闭socket */
}
/**********************************************************************/
        ...
        const char *method, const char *query_string)
{
    ...
    if (pid == 0)                               /* child: CGI script */
    {
        ...
        dup2(cgi_output[1], STDOUT);       /* 1代表着stdout，0代表着stdin，将系统标准输出重定向为cgi_output[1] */
        dup2(cgi_input[0], STDIN);         /* 将系统标准输入重定向为cgi_input[0]，这一点非常关键，cgi程序中用的是标准输入输出进行交互 */
         ...
         * 如果在调用wait()时子进程已经结束, 则wait()会立即返回子进程结束状态值. 子进程的结束状态值会由参数status 
         * 返回,而子进程的进程识别码也会一快返回.
...
int startup(u_short *port)
{
    ...
    name.sin_port = htons(*port);   /* host to network, short 短整数从主机字节序到网络字节序转换 */
    /********************************
